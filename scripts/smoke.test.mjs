...
const pages = Object.fromEntries(
  await Promise.all(
    LOCALE_CODES.map(async (code) => [
      code,
      await readFile(
        `${dist}${LOCALES[code].dir ? `/${LOCALES[code].dir}` : ''}/index.html`,
        'utf8',
      ),
    ]),
  ),
...
test('canonical, og:url and hreflang form a complete reciprocal set', () => {
  forEachLocale((code, html, locale) => {
    assert.match(
      html,
      new RegExp(`<link rel="canonical" href="${SITE}${locale.path}">`),
      `${code}: canonical`,
    )
...
test('sitemap lists every locale with its alternates', () => {
  for (const code of LOCALE_CODES) {
    assert.ok(
      sitemap.includes(`<loc>${SITE}${LOCALES[code].path}</loc>`),
      `sitemap missing ${code}`,
    )
  }
...
test('translations are complete — no locale falls back to another', () => {
  ...
  for (const code of LOCALE_CODES) {
    assert.deepEqual(
      shape(LOCALES[code]).sort(),
      reference,
      `${code} has a different content shape`,
    )
  }
})
...
test('each page ships only its own locale to the browser', () => {
  forEachLocale((code, html) => {
    ...
    for (const audience of ['drivers', 'business']) {
      assert.ok(
        runtime.audiences[audience].headline,
        `${code}: ${audience} headline missing from runtime data`,
      )
    }
  })
})
...
test('relative asset paths resolve from each locale directory', () => {
  forEachLocale((code, html, locale) => {
    ...
    for (const src of html.match(/<img [^>]*src="([^"]+)"/g) ?? []) {
      assert.ok(
        src.includes(`${prefix}assets/`),
        `${code}: asset path not relative to locale dir — ${src}`,
      )
    }
  })
})
...
test('body content is in the HTML, not only built by JS', () => {
  forEachLocale((code, html, locale) => {
    ...
    assert.ok(
      html.includes(locale.drivers.features[0].d),
      `${code}: features missing from static HTML`,
    )
    assert.ok(
      html.includes(locale.drivers.chips[2]),
      `${code}: hero chips missing from static HTML`,
    )
    assert.ok(
      html.includes(locale.pricing.plans[1].feats[0]),
      `${code}: pricing missing from static HTML`,
    )
...
test('every contact path uses the one company address', () => {
  forEachLocale((code, html) => {
    const addresses = new Set([...html.matchAll(/href="mailto:([^?"]+)/g)].map((m) => m[1]))
  ...
  for (const code of LOCALE_CODES) {
    for (const tag of ['og:image', 'twitter:image']) {
      ...
      await assert.doesNotReject(
        access(`${dist}/${url.replace(`${SITE}/`, '')}`),
        `${code}: ${tag} file missing`,
      )
    }
  }
})
...
test('fonts are self-hosted — no request reaches Google', () => {
  ...
  assert.doesNotMatch(
    ...
    'stylesheet still references Google Fonts',
  )
  forEachLocale((code, html) => {
    assert.doesNotMatch(
      html,
      /fonts\.(googleapis|gstatic)\.com/,
      `${code}: still requests Google Fonts`,
    )
...
test('every @font-face file exists and is preloaded where it is needed', async () => {
  ...
  forEachLocale((code, html, locale) => {
    for (const file of locale.fonts) {
      ...
      assert.ok(
        html.includes(
          `href="${locale.dir ? '../' : ''}assets/fonts/${file}" as="font" type="font/woff2" crossorigin`,
        ),
...
test('brand wordmark is a themeable mask, and the text stays in the DOM', async () => {
  await assert.doesNotReject(
    access(`${dist}/assets/spark-title.png`),
    'wordmark asset missing from build',
  )
  assert.match(
    css,
    /@supports \(\(mask-image:.*\) or \(-webkit-mask-image:.*\)\)/,
    'mask needs a support guard',
  )
  ...
  assert.match(
    css,
    /\.nav-brand span \{[^}]*background-color:var\(--wm\)/,
    'nav wordmark must follow the theme token',
  )
  assert.match(
    css,
  ...
  forEachLocale((code, html) => {
    assert.ok(
      html.includes('<span>sPark</span>'),
      `${code}: brand name must remain as text for screen readers`,
    )
    assert.match(
      html,
...
test('the dashboard mockup uses the wordmark too', () => {
  forEachLocale((code, html) => {
    ...
    assert.equal(
      sidebar.length,
      1,
      `${code}: expected exactly one wordmark <img> (the dashboard mockup)`,
    )
    assert.match(sidebar[0], /width="\d+" height="\d+"/, `${code}: wordmark image needs dimensions`)
    assert.doesNotMatch(
      html,
      />sPark<\/div>/,
      `${code}: dashboard sidebar still renders the brand as text`,
    )
  })
})
...
test('images inside the initially-hidden dashboard mock are not lazy', () => {
  forEachLocale((code, html) => {
    const mock =
      html.match(/<div class="dash-mock"[\s\S]*?(?=<\/div>\s*<\/div>\s*<\/section>)/)?.[0] ?? ''
...
test('long price amounts are scaled down so they cannot distort the grid', () => {
  ...
  assert.match(
    ...
    'price cards need min-width:0 or a long word inflates its column',
  )
  assert.match(
    css,
    /\.price-amt \.big\.is-long \{ font-size:clamp\(/,
    'missing the is-long size rule',
  )
  assert.match(
    css,
    /\.price-amt \.big \{[^}]*overflow-wrap:break-word/,
    'amounts need a break-word fallback',
  )
  ...
  for (const code of LOCALE_CODES) {
    for (const plan of LOCALES[code].pricing.plans) {
      ...
      const rendered = pages[code].match(
        new RegExp(
          `<span class="big([^"]*)">${plan.amount.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')}</span>`,
        ),
      )
  ...
  forEachLocale((code, html) => {
    for (const tag of html.match(/<img [^>]*>/g) ?? []) {
      assert.doesNotMatch(
        tag,
        /spark-mark\.png/,
        `${code}: inline image still uses the 103 KB original — ${tag.slice(0, 60)}`,
      )
    }
    ...
    assert.ok(
      meta(html, 'og:image').endsWith('spark-mark.png'),
      `${code}: og:image should stay the full-size mark`,
    )
  })
})
...
test('every referenced image file exists in the build', async () => {
  for (const code of LOCALE_CODES) {
    ...
    const refs = [...pages[code].matchAll(/(?:src|href)="((?:\.\.\/)?assets\/[^"]+)"/g)].map(
      (m) => m[1],
    )
    for (const ref of new Set(refs)) {
      ...
      await assert.doesNotReject(
        access(path),
        `${code}: ${ref} missing from build (dir=${dir || '/'})`,
      )
    }
  }
})
...
test('self-hosted fonts ship with their OFL licenses', async () => {
  for (const family of ['Sora', 'Manrope']) {
    ...
    await assert.doesNotReject(
      access(path),
      `${family} is self-hosted but its OFL license is missing`,
    )
...
test('Greek pages use a display font that has Greek glyphs', () => {
  assert.match(
    css,
    /html\[lang="el"\] \{ --fontDisplay: 'Manrope'/,
    'Greek must not fall back to Sora',
  )
